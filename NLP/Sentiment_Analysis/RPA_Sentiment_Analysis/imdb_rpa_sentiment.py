        ...
        try:
            ...
            for row in soup.select("tbody.lister-list tr"):
                try:
                    ...
                    rating = (
                        rating_col.strong.text.strip() if rating_col and rating_col.strong else ""
                    )
        ...
        for attempt in range(self.config.retry_attempts):
            try:
                ...
                if attempt == self.config.retry_attempts - 1:
                    logger.error(
                        f"Failed to load reviews after {self.config.retry_attempts} attempts"
                    )
                    return []
            except Exception as e:
...
class DataProcessor:
    ...
    def process_movie(
        self, movie: Dict, scraper: IMDBScraper, analyzer: SentimentAnalyzer
    ) -> MovieData:
...
"""
        ...
        if successful_results:
            sorted_results = sorted(
                successful_results, key=lambda x: x.avg_sentiment or 0, reverse=True
            )
            for i, result in enumerate(sorted_results[:5], 1):
                report += (
                    f"{i}. {result.title} ({result.year}) - Sentiment: {result.avg_sentiment:.3f}\n"
                )
...
class IMDBRPASentimentAnalysis:
    ...
    def _process_movies_parallel(self, movies: List[Dict]) -> List[MovieData]:
        ...
        with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
            future_to_movie = {
                executor.submit(
                    self.processor.process_movie, movie, self.scraper, self.analyzer
                ): movie
                for movie in movies
            }

            for future in tqdm(
                as_completed(future_to_movie), total=len(movies), desc="Processing movies"
            ):
...
def main():
    ...
    parser.add_argument(
        "--test", action="store_true", help="Run in test mode (process only 2 movies)"
    )
