...
class PerformanceMonitor:
    ...
    def record_metrics(
        self, movies_processed: int = 0, reviews_processed: int = 0, errors_count: int = 0
    ):
...
class NetworkMonitor:
    ...
    def stop_monitoring(self):
        ...
        if self.initial_network_stats:
            ...
            logger.info(
                f"Network activity: {bytes_sent:,} bytes sent, {bytes_recv:,} bytes received"
            )
...
def main():
    ...
    parser.add_argument(
        "--check-connectivity", action="store_true", help="Check network connectivity"
    )
