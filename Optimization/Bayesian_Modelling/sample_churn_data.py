...
df["MultipleLines"] = np.random.choice(
    ["Yes", "No", "No phone service"], size=n_samples, p=[0.4, 0.4, 0.2]
)
df["OnlineSecurity"] = np.random.choice(
    ["Yes", "No", "No internet service"], size=n_samples, p=[0.3, 0.5, 0.2]
)
df["OnlineBackup"] = np.random.choice(
    ["Yes", "No", "No internet service"], size=n_samples, p=[0.3, 0.5, 0.2]
)
df["DeviceProtection"] = np.random.choice(
    ["Yes", "No", "No internet service"], size=n_samples, p=[0.3, 0.5, 0.2]
)
df["TechSupport"] = np.random.choice(
    ["Yes", "No", "No internet service"], size=n_samples, p=[0.2, 0.6, 0.2]
)
df["StreamingTV"] = np.random.choice(
    ["Yes", "No", "No internet service"], size=n_samples, p=[0.4, 0.4, 0.2]
)
df["StreamingMovies"] = np.random.choice(
    ["Yes", "No", "No internet service"], size=n_samples, p=[0.4, 0.4, 0.2]
)
...
print(
    f"Contract distribution: Month-to-month: {(df['Contract']==0).mean():.1%}, One year: {(df['Contract']==1).mean():.1%}, Two year: {(df['Contract']==2).mean():.1%}"
)
print(
    f"Internet Service: No: {(df['InternetService']==0).mean():.1%}, DSL: {(df['InternetService']==1).mean():.1%}, Fiber: {(df['InternetService']==2).mean():.1%}"
)
