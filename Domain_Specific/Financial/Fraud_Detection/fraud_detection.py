...
class FraudDetectionSystem:
    ...
    def generate_synthetic_fraud_data(self, n_samples=50000):
        ...
        transaction_types = np.random.choice(
            ["purchase", "withdrawal", "transfer", "payment"], n_samples
        )
    ...
    def visualize_fraud_analysis(self, data, predictions=None):
        ...
        # Fraud by merchant category
        fraud_by_merchant = (
            data.groupby("merchant_category")["is_fraud"].mean().sort_values(ascending=False)
        )
        ...
        # Age vs Fraud
        axes[1, 0].scatter(
            data[data["is_fraud"] == 0]["customer_age"],
            data[data["is_fraud"] == 0]["amount"],
            alpha=0.6,
            label="Legitimate",
            s=20,
        ...
        axes[1, 0].scatter(
            data[data["is_fraud"] == 1]["customer_age"],
            data[data["is_fraud"] == 1]["amount"],
            alpha=0.8,
            label="Fraudulent",
            s=50,
    ...
    def generate_fraud_report(self, transaction_id, transaction_data, fraud_results):
        ...
        model_scores = {
            name: scores[0] for name, scores in fraud_results["individual_scores"].items()
        }
...
def main():
    ...
    print(
        f"Generated {len(data)} transactions with {data['is_fraud'].sum()} fraudulent transactions"
    )
