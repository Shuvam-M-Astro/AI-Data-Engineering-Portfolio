...
class PredictiveMaintenance:
    ...
    def train_rul_model(self, X, y):
        ...
        self.feature_importance = pd.DataFrame(
            {"feature": X.columns, "importance": self.rul_model.feature_importances_}
    ...
    def visualize_results(self, data, rul_predictions=None):
        ...
        # Temperature vs Time
        axes[0, 0].scatter(
            data["time_in_service"], data["temperature"], alpha=0.6, c=data["rul"], cmap="viridis"
        )
        ...
        # Vibration vs Time
        axes[0, 1].scatter(
            data["time_in_service"], data["vibration"], alpha=0.6, c=data["rul"], cmap="viridis"
        )
        ...
        # Pressure vs Current
        axes[0, 2].scatter(
            data["pressure"], data["current"], alpha=0.6, c=data["rul"], cmap="viridis"
        )
    ...
    def generate_maintenance_report(self, equipment_id, sensor_data):
        ...
        if predictions["anomalies"][0] == -1:
            ...
            report += (
                "- INSPECT: Check for mechanical failures, overheating, or electrical issues\n"
            )
