...
class PatientRiskPrediction:
    ...
    def analyze_risk_factors(self, data):
        ...
        print(
            f"Overall 30-day readmission rate: {readmission_rate:.3f} ({readmission_rate*100:.1f}%)"
        )
        ...
        risk_by_age = data.groupby("age_group")[
            ["readmission_30_days", "mortality_30_days", "complications"]
        ].mean()
        print(risk_by_age)
        ...
        for comorbidity in comorbidities:
            ...
            if comorbidity_mask.sum() > 0:
                ...
                print(
                    f"  Readmission rate: {data[comorbidity_mask]['readmission_30_days'].mean():.3f}"
                )
    ...
    def visualize_risk_analysis(self, data, predictions=None):
        ...
        # Age vs Risk
        axes[0, 0].scatter(
            data[data["readmission_30_days"] == 0]["age"],
        ...
        axes[0, 0].scatter(
            data[data["readmission_30_days"] == 1]["age"],
        ...
        axes[0, 2].hist(
            data[data["readmission_30_days"] == 0]["heart_rate"],
            bins=30,
            alpha=0.7,
            label="No Readmission",
            density=True,
        ...
        axes[0, 2].hist(
            data[data["readmission_30_days"] == 1]["heart_rate"],
            bins=30,
            alpha=0.7,
            label="Readmission",
            density=True,
    ...
    def generate_risk_report(
        self, patient_id, patient_data, risk_results, model_type="readmission"
    ):
        """
        ...
        model_scores = {
            name: scores[0] for name, scores in risk_results["individual_scores"].items()
        }
