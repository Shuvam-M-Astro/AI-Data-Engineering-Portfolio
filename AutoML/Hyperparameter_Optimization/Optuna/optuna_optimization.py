...
class AutoMLOptimizer:
    ...
    def generate_synthetic_data(
        self, n_samples=10000, n_features=20, n_classes=2, task="classification"
    ):
        """
        ...
        if task == "classification":
            ...
            X, y = make_regression(
                n_samples=n_samples, n_features=n_features, n_informative=10, random_state=42
            )
    ...
    def create_model(self, trial, model_type="random_forest"):
        ...
        if model_type == "random_forest":
            ...
            return SVC(C=C, kernel=kernel, gamma=gamma, random_state=42, probability=True)

        elif model_type == "neural_network":
            hidden_layer_sizes = trial.suggest_categorical(
                "nn_hidden_layer_sizes", [(50,), (100,), (50, 50), (100, 50), (100, 100)]
            )
    ...
    def optimize_multiple_models(
        self, X, y, model_types=None, optimization_type="single_objective"
    ):
        """
        ...
        for model_type in model_types:
            ...
            try:
                study = self.optimize_hyperparameters(X, y, model_type, optimization_type)
    ...
    def visualize_optimization_results(self, study, model_type):
        ...
        fig.suptitle(
            f'Optimization Results for {model_type.replace("_", " ").title()}', fontsize=16
        )
        ...
        try:
            ...
            axes[0, 1].text(
                0.5,
                0.5,
                "Parameter importance not available",
                ha="center",
                va="center",
        ...
        if len(study.trials) > 0 and hasattr(study.trials[0], "params"):
            ...
            if len(params) >= 2:
                ...
                values1 = [
                    trial.params.get(param1, 0) for trial in study.trials if trial.value is not None
                ]
                values2 = [
                    trial.params.get(param2, 0) for trial in study.trials if trial.value is not None
                ]
        ...
        if scores:
            ...
            axes[1, 1].axvline(
                np.max(scores), color="green", linestyle="--", label=f"Best: {np.max(scores):.4f}"
            )
