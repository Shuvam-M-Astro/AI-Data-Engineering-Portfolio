...
class DBSCANClustering:
    ...
    def find_optimal_eps(self, X, k=5, plot=True):
        ...
        if plot:
            ...
            plt.axhline(
                y=optimal_eps, color="r", linestyle="--", label=f"Optimal eps: {optimal_eps:.3f}"
            )
            plt.axvline(x=elbow_idx, color="g", linestyle="--", label=f"Elbow point: {elbow_idx}")
            plt.xlabel("Points")
    ...
    def plot_clusters(self, X, figsize=(12, 8)):
        ...
        for label, color in zip(unique_labels, colors):
            ...
            plt.scatter(xy[:, 0], xy[:, 1], c=[col], s=50, label=label_name, alpha=0.7)
...
def generate_sample_datasets():
    ...
    X1, y1 = make_blobs(
        n_samples=300, centers=4, cluster_std=0.60, random_state=42, center_box=(-10, 10)
    )
