...
from sklearn.datasets import load_iris, make_blobs, make_swiss_roll
...
class UMAPAnalysis:
    ...
    def __init__(
        self, n_components=2, n_neighbors=15, min_dist=0.1, metric="euclidean", random_state=42
    ):
        """
    ...
    def find_optimal_parameters(self, X, n_neighbors_range=None, min_dist_range=None, plot=True):
        ...
        for n_neighbors in n_neighbors_range:
            for min_dist in min_dist_range:
                try:
                    ...
                    results.append(
                        {"n_neighbors": n_neighbors, "min_dist": min_dist, "error": error}
                    )
                except:
                    continue
        ...
        if plot:
            ...
            plt.plot(
                ...
                "r*",
                markersize=15,
                label=f"Optimal: ({optimal_n_neighbors}, {optimal_min_dist})",
            )
            plt.legend()
            plt.show()
    ...
    def plot_embedding_3d(self, X, labels=None, figsize=(12, 8), title=None):
        ...
        if title is None:
            title = (
                f"UMAP 3D Embedding (n_neighbors: {self.n_neighbors}, min_dist: {self.min_dist})"
            )
...
def generate_sample_datasets():
    ...
    X2, y2 = make_blobs(
        n_samples=1000, centers=5, cluster_std=[0.5, 1.0, 0.8, 1.2, 0.6], random_state=42
    )
