...
class AdvancedCustomerSegmentation:
    ...
    def generate_sample_data(self, n_customers=1000):
        ...
        self.data["days_since_last_purchase"] = np.clip(
            np.abs(self.data["days_since_last_purchase"]), 1, 365
        )
    ...
    def find_optimal_parameters(self, algorithm="kmeans", max_clusters=10):
        ...
        if algorithm == "kmeans":
            ...
            param_grid = {"eps": np.arange(0.1, 2.0, 0.1), "min_samples": [3, 5, 10, 15]}
    ...
    def compare_algorithms(self, max_clusters=10):
        ...
        best_algorithm = max(self.models.keys(), key=lambda x: self.models[x]["score"])
    ...
    def visualize_algorithm_comparison(self, save_path=None, show=True):
        ...
        silhouette_scores = [self.models[alg]["metrics"]["silhouette_score"] for alg in algorithms]
        n_clusters = [self.models[alg]["metrics"]["n_clusters"] for alg in algorithms]
        calinski_scores = [
            self.models[alg]["metrics"]["calinski_harabasz_score"] for alg in algorithms
        ]

        # Algorithm comparison
        fig.add_trace(
            go.Bar(x=algorithms, y=silhouette_scores, name="Silhouette Score"), row=1, col=1
        )

        # Silhouette scores
        fig.add_trace(go.Bar(x=algorithms, y=silhouette_scores, name="Silhouette"), row=1, col=2)

        # Number of clusters
        fig.add_trace(go.Bar(x=algorithms, y=n_clusters, name="Clusters"), row=2, col=1)

        # Calinski-Harabasz scores
        fig.add_trace(
            go.Scatter(
                x=algorithms, y=calinski_scores, mode="markers+lines", name="Calinski-Harabasz"
            ),
            row=2,
            col=2,
        )

        fig.update_layout(title_text="Clustering Algorithm Comparison", height=600, showlegend=True)
...
def run_experiment(cfg):
        ...
        summary[alg] = {
            ...
            "metrics": {
                ...
                "calinski_harabasz_score": float(
                    info["metrics"].get("calinski_harabasz_score", -1)
                ),
    ...
    if mlflow is not None:
        try:
            ...
            mlflow.log_metric(
                "calinski_harabasz_score", float(best_metrics["calinski_harabasz_score"])
            )
