...
class HierarchicalClustering:
    ...
    def plot_clusters(self, X, figsize=(12, 8)):
        ...
        for label, color in zip(unique_labels, colors):
            ...
            plt.scatter(xy[:, 0], xy[:, 1], c=[color], s=50, label=f"Cluster {label}", alpha=0.7)
    ...
    def find_optimal_clusters(self, X, max_clusters=10, plot=True):
        ...
        for n_clusters in n_clusters_range:
            ...
            model = AgglomerativeClustering(
                n_clusters=n_clusters, linkage=self.linkage, metric=self.metric
            )
        ...
        if plot:
            ...
            ax2.axvline(
                x=optimal_calinski, color="r", linestyle="--", label=f"Optimal: {optimal_calinski}"
            )
    ...
    def compare_linkage_methods(self, X, linkage_methods=None, figsize=(15, 10)):
        ...
        for i, linkage_method in enumerate(linkage_methods):
            ...
            for label, color in zip(unique_labels, colors):
                ...
                xy = X_scaled[class_member_mask]
                axes[i].scatter(
                    xy[:, 0], xy[:, 1], c=[color], s=50, label=f"Cluster {label}", alpha=0.7
                )
...
def generate_sample_datasets():
    ...
    X1, y1 = make_blobs(n_samples=300, centers=4, cluster_std=[0.5, 1.0, 0.8, 1.2], random_state=42)
