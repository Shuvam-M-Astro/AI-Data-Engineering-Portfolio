...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("ab_testing.log"), logging.StreamHandler(sys.stdout)],
)
...
class ABTestRunner:
    ...
    def create_optimizer(
        self, optimizer_config: Dict, model: nn.Module, lr: float
    ) -> optim.Optimizer:
    ...
    def create_scheduler(
        self, scheduler_config: Dict, optimizer: optim.Optimizer, train_loader: DataLoader
    ) -> Any:
    ...
    def get_data_loaders(
        self, train_transform, test_transform, batch_size: int
    ...
    def train_model(
        ...
        for epoch in range(self.config.max_epochs):
            ...
            for inputs, labels in tqdm(
                trainloader, desc=f"Epoch {epoch+1}/{self.config.max_epochs}"
            ):
    ...
    def save_model(
        self, model: nn.Module, config_name: str, run_id: int, epoch: int, accuracy: float
    ):
    ...
    def run_single_config(self, config: Dict) -> List[Dict[str, Any]]:
        ...
        for run_id in range(self.config.num_runs_per_config):
            ...
            reg_config = next(
                r for r in REGULARIZATION_CONFIGS if r["name"] == config["regularization"]
            )
            ...
            result = self.train_model(
                model, trainloader, testloader, optimizer, scheduler, criterion, config_name, run_id
            )
...
"""
        ...
        # Model analysis
        model_analysis = (
            df.groupby("model")["best_accuracy"]
            .agg(["mean", "std"])
        ...
        # Optimizer analysis
        optimizer_analysis = (
            df.groupby("optimizer")["best_accuracy"]
            .agg(["mean", "std"])
            .sort_values("mean", ascending=False)
        )
        report += "\nOptimizers:\n"
        ...
        # Scheduler analysis
        scheduler_analysis = (
            df.groupby("scheduler")["best_accuracy"]
            .agg(["mean", "std"])
            .sort_values("mean", ascending=False)
        )
        report += "\nSchedulers:\n"
        ...
        significant_pairs = [
            k for k, v in stats_analysis["pairwise_tests"].items() if v["significant"]
        ]
        report += f"\nStatistical Significance Summary:\n"
    ...
    def calculate_statistical_significance(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for i, config1 in enumerate(configs):
            for config2 in configs[i + 1 :]:
                ...
                pooled_std = np.sqrt(
                    (
                        (len(acc1) - 1) * np.var(acc1, ddof=1)
                        + (len(acc2) - 1) * np.var(acc2, ddof=1)
                    )
                    / (len(acc1) + len(acc2) - 2)
                )
        ...
        return {"confidence_intervals": significance_results, "pairwise_tests": pairwise_tests}
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="ab_test_results", help="Output directory"
    )
