...
from shared_utils.reproducibility import get_torch_generator, seed_worker, set_global_seed
...
class MNISTDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self, root: str, train: bool = True, transform: Optional[transforms.Compose] = None
    ):
...
def parse_args():
    ...
    parser.add_argument(
        "--batch_size", type=int, default=128, help="Batch size for training/inference"
    )
...
def prune_model(model: nn.Module, amount: float = 0.3) -> nn.Module:
    ...
    try:
        # Create pruner
        pruner = L1Unstructured(amount=amount, module_type=nn.Conv2d)
...
def inference(
    model: nn.Module, image: torch.Tensor, device: torch.device
    ...
    try:
        ...
        if args.mode == "train":
            ...
            if writer:
                ...
                writer.add_pr_curve(
                    "Inference/PR_Curve", torch.tensor(predictions), torch.tensor(labels)
                )
