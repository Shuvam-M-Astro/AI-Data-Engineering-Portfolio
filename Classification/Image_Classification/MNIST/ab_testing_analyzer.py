...
class ABTestAnalyzer:
    ...
    def load_results(self):
        ...
        print(
            f"Loaded {len(self.df)} results from {len(self.df['config_name'].unique())} configurations"
        )

    def basic_statistics(self) -> pd.DataFrame:
        """Calculate basic statistics for each configuration."""
        stats_df = (
            self.df.groupby("config_name")
            .agg(
                {
    ...
    def factor_analysis(self) -> Dict[str, pd.DataFrame]:
        ...
        for factor in factors:
            if factor in self.df.columns:
                analysis = (
                    self.df.groupby(factor)["best_accuracy"]
    ...
    def statistical_significance_test(self, config1: str, config2: str) -> Dict[str, float]:
        ...
        pooled_std = np.sqrt(
            ((len(acc1) - 1) * np.var(acc1, ddof=1) + (len(acc2) - 1) * np.var(acc2, ddof=1))
            / (len(acc1) + len(acc2) - 2)
        )
    ...
    def create_factor_impact_plot(self, save_path: str = None):
        ...
        for i, factor in enumerate(factors):
            if factor in self.df.columns:
                ...
                bars = axes[i].bar(
                    range(len(factor_data)),
                    factor_data["mean"],
                    yerr=factor_data["std"],
                    capsize=5,
                    alpha=0.7,
                )
    ...
    def create_performance_heatmap(self, save_path: str = None):
        ...
        pivot_data = self.df.pivot_table(
            values="best_accuracy", index="model", columns="optimizer", aggfunc="mean"
        )

        plt.figure(figsize=(10, 8))
        sns.heatmap(
            pivot_data, annot=True, fmt=".3f", cmap="RdYlGn", center=pivot_data.mean().mean()
        )
    ...
    def create_confidence_intervals(self, save_path: str = None):
        ...
        for i, config in enumerate(top_configs):
            ...
            plt.errorbar(
                i, mean_acc, yerr=ci_95, fmt="o", capsize=5, capthick=2, markersize=8, label=config
            )
    ...
    def generate_comprehensive_report(self, output_dir: str = "analysis_results"):
        ...
        if len(top_configs) > 0:
            self.create_learning_curves(
                top_configs[0], output_path / "top_config_learning_curves.png"
            )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="analysis_results", help="Output directory for analysis"
    )
