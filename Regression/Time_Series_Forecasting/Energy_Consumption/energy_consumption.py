...
class ForecastConfig:
    ...
    n_lags: int = 48
    test_size: int = 336  # last N points (two weeks for hourly data)
    random_state: int = 42
    n_estimators: int = 400
    max_depth: Optional[int] = None


def generate_synthetic_energy(n_points: int = 2000) -> pd.DataFrame:
    rng = np.random.default_rng(42)
    dates = pd.date_range(start="2020-01-01", periods=n_points, freq="H")

    daily_pattern = 15 + 5 * np.sin(2 * np.pi * np.arange(n_points) / 24)
    weekly_pattern = 3 * np.sin(2 * np.pi * np.arange(n_points) / (24 * 7))
    temperature_effect = 0.05 * (30 - 10 * np.sin(2 * np.pi * np.arange(n_points) / (24 * 365)))
    noise = rng.normal(0, 1.0, n_points)

    value = daily_pattern + weekly_pattern + temperature_effect + noise
...
def train_and_evaluate(config: ForecastConfig) -> None:
    ...
    print("Energy Consumption Forecasting Results")
    print("-" * 40)
