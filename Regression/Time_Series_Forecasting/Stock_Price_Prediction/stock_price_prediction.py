...
class StockPricePredictor:
    def __init__(
        self, symbol="AAPL", start_date="2020-01-01", end_date="2024-01-01", seed: int = 42
    ):
        """
    ...
    def evaluate_model(self, X_test, y_test, data_clean):
        ...
        y_test_original = self.scaler.inverse_transform(
            np.concatenate(
                [y_test.reshape(-1, 1), np.zeros((len(y_test), data_clean.shape[1] - 1))], axis=1
            )
        )[:, 0]
        ...
        future_predictions_original = self.scaler.inverse_transform(
            np.concatenate(
                [
                    ...
                    np.zeros((len(future_predictions), 10)),
                ],
                axis=1,
            )
        )[:, 0]
